...
function main(;n=3, Plotter = PyPlot, tend = 20.0, unknown_storage=:sparse,
              reactionN1 = 5.e7, reactionP1 = 5.e7,     # "small" jumps
              reactionN2 = 5.e7, reactionP2 = 5.0e7)
    ...
    ipsiD    = ContinuousQuantity(sysD, 1:numberOfRegions,    id = 3)
    ...
    function breaction!(f, u, bnode)
        ...
        if bnode.region == bregionJunction1
            ...
            reactN = reactionN1 * (nleft  - nright)
            f[iphinD, 1] = reactN
            f[iphinD, 2] = -reactN # minus from normal vector cancels out with sign from brackets

            @show reactN.value
            # left and right reaction for p
            reactP = reactionP1 * (pleft  - pright)
            f[iphipD, 1] = reactP
            f[iphipD, 2] = - reactP # minus from normal vector cancels out with sign from brackets
