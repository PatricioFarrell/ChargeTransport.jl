...
function main(;n = 13, Plotter = PyPlot, plotting = false, verbose = false, test = false, unknown_storage=:dense)
    ...
    if plotting
        dfusion_grid          = 1.0e-2.*readdlm("with-interface/Driftfusion-pedotpss-grid.dat")

        dfusion_psi_interface = readdlm("with-interface/Driftfusion-pedotpss-Na-1p21e22-interface-reco-psi-EQ.dat")
        ##########################
    ...
    for istep = 2:ntsteps
        ...
        if plotting
            dfusion_psi           = readdlm("with-interface/Driftfusion-pedotpss-Na-1p21e22-psi-t-30.dat")

            dfusion_phin   = readdlm("with-interface/Driftfusion-pedotpss-Na-1p21e22-Efn-t-30.dat")
            dfusion_phip   = readdlm("with-interface/Driftfusion-pedotpss-Na-1p21e22-Efp-t-30.dat")

            dfusion_psi_interface = readdlm("with-interface/Driftfusion-pedotpss-Na-1p21e22-interface-reco-psi-t-30.dat")
            dfusion_phin_interface   = readdlm("with-interface/Driftfusion-pedotpss-Na-1p21e22-interface-reco-Efn-t-30.dat")
            dfusion_phip_interface   = readdlm("with-interface/Driftfusion-pedotpss-Na-1p21e22-interface-reco-Efp-t-30.dat")
            ##########################
            plot_solution(Plotter, ctsys, solution)
