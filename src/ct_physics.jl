...
will be deleted in future version !!! Part with internal boundaries needs to be tested.
"""

function breactionOhmicSurfaceReco!(f, u, bnode, data)
    ...
    if data.innerInterfaces == true # convention: inner interfaces are boundary 3 and 4
        ...
        if ireg == 3
            ...
            if data.inEquilibrium == true
                ...
                n                     = compute_densities!(u, data, inode, ireg, iphin, ipsi, false)  # false for boundary region
                p                     = compute_densities!(u, data, inode, ireg, iphip, ipsi, false) 
                exponentialTerm       = exp((q *u[iphin] - q  * u[iphip]) / (kB*params.temperature))
                excessCarrierDensTerm = n*p * (1.0 - exponentialTerm)
    
                # surface SRH recombination
                 kernelSRH       = 1.0 / (  1.0/params.recombinationSRHvelocity[iphip, ireg] * (n + params.brecombinationSRHTrapDensity[iphin, ireg]) + 1.0/params.recombinationSRHvelocity[iphin, ireg] * (p + params.brecombinationSRHTrapDensity[iphip, ireg]) )

                f[iphin]          =  - q * kernelSRH *  excessCarrierDensTerm 
                f[iphip]          =    q * kernelSRH *  excessCarrierDensTerm
