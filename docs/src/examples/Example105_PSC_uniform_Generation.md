...
([source code](https://github.com/PatricioFarrell/ChargeTransportInSolids.jl/tree/master/examplesExample105_PSC_uniform_Generation.jl))
...
function main(;n = 13, Plotter = true, plotting = false, verbose = false, test = false, unknown_storage=:dense)
...
```julia
    generation_a = 0.0
    generation_i = 0.0 #2.64e21 / (cm^3 * s)
    generation_d = 0.0

    generationAbsorption_i = 2.078826021188540e7  / (m)
    generationIncidentPhotonFlux_i = (4.235553944562899e21)  / (m^2 * s)

    generationUniform = [generation_a, generation_i, generation_d]


    generationAbsorption = [0.0, generationAbsorption_i, 0.0]
    generationIncidentPhotonFlux = [0.0, generationIncidentPhotonFlux_i, 0.0]
```
...
```julia
        ...
        data.generationUniform[ireg]                  = generationUniform[ireg]
        data.generationAbsorption[ireg]               = generationAbsorption[ireg]
        data.generationIncidentPhotonFlux[ireg]       = generationIncidentPhotonFlux[ireg]
    end
```
