...
# ╔═╡ f2317302-b4ce-4169-a1ef-3ce6dcc1c63d
md"""
once in main branch, make pluto notebook compatible with latest changes:
- boundary conditions
- get_density() calculation
"""

# ╔═╡ 279aad43-4c0c-4463-9b1f-9d2192785a77
md"""
Make clear that
- this is an illustrative/ a theoretical example for the more general charge transport model in multi-dimensions, different materials, different scan protocols, different boundary conditions, etc ... [point to the docs part with charge transport model]
- We have an ohmic contact
- should show the capability and user-friendliness of code in an interactive way
"""
...
md"""
For illustrative purposes we use in the following a one-dimensional non-uniform grid with $(length(coord)) number of nodes. The used parameters are estimated, but shall represent the following set-up:
...
md"""
We consider the device at a constant temperature T = 298 K.
"""
...
md"""
	Set bulk recombination $(@bind isBulkRecoOn Select(["true" => "on", "false" => "off"], default="true"))
	"""
...
md"""
With this predefined information a corresponding finite volume discretiaztion of the model is set.
"""
...
begin
	...
	#### boundary information, can be deleted, once in main branch
...
# ╔═╡ 11e7ffd1-7917-4b3d-88cf-1ba9ad79683a
md"""
Further, the solution and the respective I-V characteristics are calculated.
"""
...
# ╔═╡ 4c366ed2-74a1-41d9-a7c8-2f4f13a3099f
begin
...
# ╔═╡ f0be8095-38a6-4ef5-be89-1f278f332ea0
begin
...
# ╔═╡ 4d4e3c5a-a84b-4d6d-8197-272852f4a626
begin
...
# ╔═╡ e50df2ba-bb27-4bc5-966f-42413f22e09d
begin
...
# ╔═╡ c2a8e58d-453e-4aec-9101-896434ca3188
...
begin
    ...
    efficiency                =  bias[indexPD] * -IV[indexPD]  / (IncidentLightPowerDensity * (0.1 * 0.1) * m^2) * 100 
    fillfactor                = (bias[indexPD] * -IV[indexPD]) / (-IV[1] * open_circuit) * 100
	
	PyPlot.clf()
	PyPlot.plot(biasValues[2:end], -IV*(0.01^2), linewidth = 5, label = "forward")
	PyPlot.plot(biasValuesReverse[2:end], -IVReverse*(0.01^2), linestyle = ":", linewidth = 5, label = "reverse")
	
	#PyPlot.plot(biasValues[2:end], powerDensity*(0.01^2), linewidth = 3, label = "power density", linestyle = ":", color = "gray")
	PyPlot.grid()
	PyPlot.legend()
...
md"""
As an illustrative example the band gap in the perovskite material is varied and the efficiency, open circuit voltage and the fill factor are calculated, respectively.
"""
...
begin
    ...
    for Eg in EgTest
        ...
        local IncidentLightPowerDensity = 1000.0 * W/m^2 # for one sun

    local efficiency                =  bias[indexPD] * IV[indexPD]  / (IncidentLightPowerDensity * (0.1 * 0.1) * m^2) * 100 
...
julia_version = "1.8.3"
manifest_format = "2.0"
project_hash = "4de6adc80b6cede3efc7ba239749ea1474968a25"
...
git-tree-sha1 = "873c031d6c0e2480bcf097a7eb8904b32ab6128d"
